...
function SignupForm() {
  ...
  const googlePending = searchParams.get("google_pending");
  ...
  async function finishSignup(url: string, body: Record<string, string>) {
    try {
      setLoading(true);
  ...
  function validateCurriculumSelection(): string | null {
    if (!board || !target || (board === "STATE" && !stateBoardName)) {
      return "Please complete your educational selection.";
    }
    return null;
  }
  ...
  function currentGrade(): string {
    ...
    return `${finalBoard}-${target}`.toUpperCase();
  }

  async function handleGoogleSignup() {
    setError(null);
    const validationError = validateCurriculumSelection();
    if (validationError) { setError(validationError); return; }
    if (!name.trim()) { setError("Please enter your name."); return; }

    await finishSignup(getApiUrl("/api/auth/google/complete-signup"), {
      token: googlePending as string,
      name: name.trim(),
      grade: currentGrade(),
    });
  }
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    await finishSignup(getApiUrl("/api/auth/signup"), { name, email, password, grade: currentGrade() });
  }
        ...
        <div style={{ ...authCardStyle, boxShadow: "0 4px 6px rgba(0,0,0,0.04), 0 20px 60px rgba(79,124,255,0.10), inset 0 1px 0 rgba(255,255,255,0.6)" }}>
          ...
          <div style={{ textAlign: "center", marginBottom: 24 }}>
            <div style={{
              ...
              {googlePending ? "🎓" : step === 1 ? "✨" : "🔐"}
            </div>
            <h1 style={{ fontSize: 23, fontWeight: 800, letterSpacing: "-0.03em", marginBottom: 5, color: C.text }}>
              {googlePending ? "Almost done" : step === 1 ? "Create your account" : "Set your password"}
            </h1>
            <p style={{ fontSize: 13, color: C.muted }}>
              {googlePending
                ? "Tell us your class/board so we can personalize your curriculum"
            ...
            {(step === 1 || googlePending) && (
              <div style={{ display: "flex", flexDirection: "column", gap: 16, animation: "slideIn 0.3s ease both" }}>
                ...
                <div>
                  ...
                  <input
                    className="vs-input"
                    style={{ ...inputStyle, ...(googlePending ? { opacity: 0.65, cursor: "not-allowed" } : {}) }}
                    type="email"
                  ...
                  />
                  {googlePending && (
                    <p style={{ fontSize: 12, color: C.muted, marginTop: 5 }}>✓ Verified with Google</p>
                  )}
                </div>
                ...
                <button
                  ...
                  style={{ marginTop: 4 }}
                  onClick={() => {
                    if (googlePending) { handleGoogleSignup(); return; }
                    if (!name.trim()) { setError("Please enter your name."); return; }
                ...
                >
                  {googlePending ? (loading ? "Creating account…" : "Finish sign-up →") : "Continue →"}
                </button>
